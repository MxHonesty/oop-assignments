...
void AddDialog::init() {
	...
	denumire_layout->addRow("Destinatie", destinatie);
	denumire_layout->addRow("Tip", tip);
	denumire_layout->addRow("Pret", pret);
}

void AddDialog::add_oferta(const Oferta& oferta) {
}
...
void AddDialog::connect_signals() {
	QObject::connect(btn_submit, &QPushButton::clicked, this, [this]() {
		try {
			...
			QMessageBox::warning(this, "Titlu", e.what());
		}
